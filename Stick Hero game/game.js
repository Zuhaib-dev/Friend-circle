...
let lastTimestamp; // Previous requestAnimationFrame timestamp
...
function draw() {
  ...
  drawBackground();
  ctx.translate((window.innerWidth - canvasWidth) / 2 - sceneOffset,
...
function drawPlatforms() {
  platforms.forEach(({ x, w }) => {
    ...
    if (sticks.last().x < x) {
      ctx.fillStyle = "red";
      ctx.fillRect(x + w / 2 - perfectAreaSize / 2, canvasHeight - platformHeight,
                   perfectAreaSize, perfectAreaSize);
    }
  });
}
...
function getHillY(windowX, baseHeight, amplitude, stretch) {
  ...
  return Math.sinus((sceneOffset * backgroundSpeedMultiplier + windowX) * stretch) *
    amplitude + sineBaseY;
}
