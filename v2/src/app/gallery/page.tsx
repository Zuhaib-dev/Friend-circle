...
export default function GalleryPage() {
  ...
  return (
    <main className="min-h-screen bg-bone text-ink relative">
      ...
      {/* Lightbox */}
      <AnimatePresence>
        {active && <Lightbox frame={active} onClose={() => setActive(null)} now={now} mounted={mounted} />}
      </AnimatePresence>
...
function Lightbox({
  ...
  return (
    ...
    >
      ...
      >
        <header className="hairline-b border-ink flex flex-shrink-0 items-center justify-between px-3 py-2 mono-label">
        ...
        <footer className="hairline-t border-ink grid grid-cols-2 md:grid-cols-4 mono-label text-[10px] flex-shrink-0">
          <DataCell label="UPLOADER" value={frame.uploader} />
          <DataCell label="DATE" value={mounted ? timeAgo(frame.date, now) + " AGO" : "—"} />
          ...
          <a
            ...
            className="px-2.5 py-1.5 flex items-center justify-center gap-1.5 brick text-bone hover:bg-signal transition-colors"
          >
