    ...
    <section className="px-4 md:px-8 py-8">
      <header className="flex items-end justify-between gap-4 mb-4 flex-wrap">
        <div className="flex items-baseline gap-3 min-w-0">
          <span className="brick text-bone px-2 py-[1px] mono-label">{code}</span>
...
export function Panel({
  ...
  return (
    <motion.div
      ...
      className="hairline border-ink bg-bone crosshair relative h-full"
    >
      <Crosshairs />
      <header className="hairline-b border-ink flex items-center justify-between px-2.5 py-1.5">
        <span className="flex items-center gap-2">
          <span className="brick text-bone px-1.5 py-[1px] mono-label">{code}</span>
