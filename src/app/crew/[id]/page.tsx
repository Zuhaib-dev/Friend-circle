  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (user && 'ok' in followRes && followRes.ok) {
          const data = await (followRes as any).json();
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-bone text-ink flex flex-col">
        <TopNav />
        <div className="flex-1 flex items-center justify-center p-6 text-signal mono-label animate-pulse">
          FETCHING DOSSIER...
        </div>
        <BottomNav />
      </main>
