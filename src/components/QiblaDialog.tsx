...
import { Compass, Crosshair, MapPin, Target, AlertTriangle, X } from "lucide-react";
...
export function QiblaDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Turn off sensors when modal is closed
  useEffect(() => {
    if (!open) {
      setActive(false);
    }
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {!active ? (
              <div className="relative z-10 flex flex-col items-center text-center max-w-sm mt-8">
                ...
                <p className="text-white/60 font-mono text-xs mb-4 leading-relaxed">
                  INITIATE TACTICAL HUD TO CALCULATE BEARING TO MAKKAH. REQUIRES LOCATION AND COMPASS PERMISSIONS.
                </p>
                <p className="text-[#3C8C6E]/80 font-mono text-[10px] mb-8 uppercase">
                  Tip: Wave your phone in a figure-8 to calibrate the compass for maximum accuracy.
                </p>
                {error && (
